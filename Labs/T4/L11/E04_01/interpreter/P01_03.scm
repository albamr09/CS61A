...
;;;;;;;;;;;;;;;;;;;;;;;;;
;; E01_06: create the make-unbound! method
;; that removes a symbol from a given environment
    ...
    (if (eq? env the-empty-environment)
      ...
      (let 
        ...
        (let
          ...
          (if (not (null? filtered-bindings))
            (begin
              ; Update the first part
              (set-car! frame (car filtered-bindings))
              ; Check if there are at least two elements
              (if (not (null? (cdr filtered-bindings)))
                (set-cdr! frame (cdr filtered-bindings))
                ; If not, then set the last part to null
                (set-cdr! frame nil)
              )
            )
          )
          ; Keep removing from rest of frames
          ; (env-loop (enclosing-environment env))
        )
      )
    )
