...
;;;;;;;;;;;;;;
;; E04_01: Change first-operand and rest-operands 
;; so the list-of-values evaluates the arguments from
;; left to right or right to left
;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; LEFT TO RIGHT FIRST OPERAND
;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Obtain first operand: force evaluate left to right
; >>> (define (first-operand ops) (list-ref ops 0))
;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; RIGHT TO LEFT FIRST OPERAND
;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Obtain first operand: force evaluate right to left
; first operand is last element of the list (list-ref indexes from 0
; that is why we substract 1 to the length of ops)
(define (first-operand ops) (list-ref ops (- (length ops) 1)))

; Obtain first operand: force evaluate left to right
(define (rest-operands ops) 
  (define (loop compare update index limit)
    ; While the index is in the list
    (if (compare index limit) 
      (cons
        ; Obtain first element
        (list-ref ops index)
        ; Keep taking the next elements
        ; Update the index with the given
        ; update operation (+ or -)
        (loop compare update (update index 1) limit)
      )
      ; If the index is not on the list, return empty list
      '()
    )
  )
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ;;; LEFT TO RIGHT REST OPERANDS
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ; Start iterating left to right
  ; (loop
  ;   <=
  ;   +
  ;   1
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; E01_02: evaluate the let expression as a derivation
;; LET
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; Check if the first element is "let"
(define (let? exp) (tagged-list? exp 'let))

; The parameters are a list, which are the second element of the exp
; for each element (list) of the list to extract the parameter 
; get the first element
(define (let-parameters exp) (map car (cadr exp)))

; The expressions associated with each parameter are a list, which are 
; the second element of the exp, for each element (list) of the list 
; to extract the expression get the second element
(define (let-arguments exp) (map cadr (cadr exp)))

; The body is the third element of the expression 
(define (let-body exp) (cddr exp))

; Transform a let expression into a lambda expression
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; E01_03: evaluate the let* expression as a derivation using let
;; LET
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define (make-let parameters body)
  (list 
    ; Tag of expresion type
    'let 
    ; List of pairs of parameters-arguments
    parameters 
    ; Body
    body
  )
...
; The pairs of parameter-argument is the second element of the expression 
(define (let*-parameters exp) (cadr exp))

; The body is the third element of the expression 
(define (let*-body exp) (caddr exp))

; Transform the let* expression into nested let expressions
...
(define (expand-let-parameters parameters body)
  (let
    (
      (first (car parameters))
      (rest (cdr parameters))
    )
    ; If the "first" parameter is the last one
    (if (null? rest)
      ; Create the let, whose body is the let* body
      (make-let
        (list first)
        body
      )
      ; Else create a let, whose body is the next let
      (make-let
        (list first)
        ; Create nested let
        (expand-let-parameters 
