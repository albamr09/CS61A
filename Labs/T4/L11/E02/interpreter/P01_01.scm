...
(define (type-check predicates arguments env)
  (cond
    ((null? arguments) 'okay)
    ((null? predicates) (error "Too many arguments: TYPE-CHECK"))
    ; If there is a predicate for the current argument
    ((not (null? (car predicates)))
      (let
        ((result 
          (my-apply 
            ; Operator
        ...
        (if result
          ; Keep checking
          (type-check (cdr predicates) (cdr arguments) env)
          ; Else throw error
          (error "Error: wrong argument type -- " (car arguments) ": TYPE-CHECK")
        )
      )
    )
    ; Else keep checking the rest
    (else
      (type-check (cdr predicates) (cdr arguments) env)
    )
  )
...
(define (my-apply procedure arguments)
  ...
  (cond 
    ...
    ((compound-procedure? procedure)
      ; Type checking
      (type-check 
        ; Extract predicates arguments have to satisfy
      ...
      (eval-sequence
        ...
        ; where each formal parameter is bound to the corresponding argument
        (extend-environment
