...
(define (eval exp env)
  (cond 
    ...
    ((cond? exp) 
      ; E04_05: Check if it is the extended syntax
      (if (extended-cond? exp)
        ; if it is apply special evaluation rule
        (eval-extended-cond (extended-cond-clauses exp) env)
        ; if it is not, evaluate cond normally
        (eval (cond->if exp) env)
      ) 
    )
    ; If it is an and expression apply special evaluation rule
    ((and? exp) (eval-and (clauses exp) env))
    ; If it is an or expression apply special evaluation rule
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; E04_04: 
;; Add special evaluation rules for "and" and "or"
;; expressions

(define (eval-and clauses env)
  ; If it is the last clause evaluate and do not call recursively
  (if (last-clause? clauses)
    (eval (first-clause clauses) env)
    ; Else evaluate clause and if true, keep evaluating
    (if (eval (first-clause clauses) env)
...
(define (eval-or clauses env)
  ...
  (if (last-clause? clauses)
    (eval (first-clause clauses) env)
    ; Else evaluate clause and if false, keep evaluating
    (if (not (eval (first-clause clauses) env))
      (eval-and (rest-clauses clauses) env)
      ; If true, return it
      #t
    )
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; E04_05: 
;; Add special evaluation rules for "cond" 
;; expressions

(define (eval-extended-cond clauses env)
  ...
  (if (null? clauses)
    ...
    (let 
      (
        (first (extended-cond-first-clause clauses))
        (rest (extended-cond-rest-clauses clauses))
      )
      ; If it is an else
      (if (extended-cond-else-clause? first)
        ...
        (if (null? rest)
          ...
          ; Else error
          (error "ELSE clause isn't last: EVAL-EXTENTED-COND" clauses)
        )
        ; Else: evaluate if
        (let
          ((result-test (eval (extended-cond-predicate first) env)))
          ; if true
          (if result-test
            ; apply procedure 
            (my-apply
              ; Evaluate procedure
              (eval (extended-cond-actions first) env)
              ; Pass arguments as a list
              (list result-test)
            )
            ; else keep evaluating next clause
            (eval-extended-cond (cdr clauses) env) 
          )
        )
      )
    )
