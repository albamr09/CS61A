...
;--
; 6. (3J 3J)
; And each time a combination is generated, we calculate its score
...
(define (generate-combinations-backwards hand i best)
  (cond
    ...
    ((= i 0) 
      ; Before check that the total is better than the best
      (let
        ((total (best-total-helper hand (count hand) 0)))
        (if (> total best)
          ; Update best
          total
          ; Return best until now
          best
        )
      )
    )
