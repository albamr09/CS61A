...
(define (eval exp env)
  (cond 
    ...
    ((application? exp)
      ...
      (my-apply 
        ; Force evaluation (because of lazy evaluation)
        (actual-value
          (operator exp)
          env
        )
        ; Do not evaluate operands (lazy evaluation)
        (operands exp)
        env
      )
    )
  ...
  (cond 
    ...
    ((primitive-procedure? procedure)
      (apply-primitive-procedure 
        procedure 
        ; Force evaluation of arguments (because of lazy evaluation)
        (list-of-arg-values 
          arguments
          env
        )
...
(define (eval-if exp env)
  ...
  ; Force evaluation of predicate with actual-value
